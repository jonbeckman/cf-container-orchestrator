 ...
 */
export interface ContainerOperatorEnv extends ManagedContainerEnv {
	/**
	 * DurableObject namespace for managed containers.
	 * The actual binding name is configurable via `containerNamespaceKey`.
	 * This is typed as index signature to allow any binding name.
	 */
	[key: string]: unknown;
}
 ...
 */
export interface ContainerOperatorInterface<TEnv extends ContainerOperatorEnv>
...
export function createContainerOperator<TEnv extends ContainerOperatorEnv>(
	...
	env: TEnv,
) => ContainerOperatorInterface<TEnv> {
	...
	return class ContainerOperatorImpl extends DurableObject<TEnv> {
		...
		constructor(ctx: DurableObjectState, env: TEnv) {
			...
			ctx.blockConcurrencyWhile(async () => {
				const containerFleet = await ctx.storage.get<
					[string, ContainerRecord][] | undefined
				>("containers");
				if (containerFleet) {
				...
				if (this.#config.reconcileIntervalMs > 0) {
					...
					if (!existingAlarm) {
						await ctx.storage.setAlarm(
							Date.now() + this.#config.reconcileIntervalMs,
						);
					}
				}
			});
				...
				Effect.runPromise(
					this.#containerService
						...
						.pipe(
							Effect.catchAll((err) => {
								console.error(
									`[ContainerOperator] Failed to auto-restart ${record.name}:`,
									err,
								);
								return Effect.void;
							}),
		...
		async handleLifecycleEvent(event: LifecycleEvent): Promise<void> {
			await Effect.runPromise(
				this.#lifecycleService.handleEvent(event).pipe(
					Effect.catchAll((err) => {
						console.error(
							"[ContainerOperator] Lifecycle event handling failed:",
							err,
						);
						return Effect.void;
					}),
		...
		async startContainer(
			...
			const effect = this.#containerService.start(
				config.name,
				config.config,
				restartPolicy,
				force,
		...
		async reconcile(): Promise<void> {
			const effect = this.#reconcileService.reconcile().pipe(
				Effect.catchAll((err) => {
					console.error(
						"[ContainerOperator] Manual reconciliation failed:",
						err,
					);
 ...
 */
export type ContainerOperatorClass<TEnv extends ContainerOperatorEnv> =
	ReturnType<typeof createContainerOperator<TEnv>>;

/**
 * Instance type for the operator.
 */
export type ContainerOperatorInstance<TEnv extends ContainerOperatorEnv> =
	InstanceType<ContainerOperatorClass<TEnv>>;
