...
import type { ContainerStartConfigOptions, StopParams } from "@cloudflare/containers"
import { Container } from "@cloudflare/containers"
import type { ContainerOperatorEnv, ContainerOperatorInstance } from "@/ContainerOperator"
import type { ManagedContainerState } from "./types/ContainerState"
import { formatDuration } from "./types/ContainerState"
import type { StopReason } from "./types/LifecycleEvent"
 ...
 */
export interface ManagedContainerEnv<TEnv extends ContainerOperatorEnv> {
  /** DurableObject namespace for the operator */
  CONTAINER_OPERATOR: DurableObjectNamespace<ContainerOperatorInstance<TEnv>>
}
...
export abstract class ManagedContainer<
  TEnv extends ManagedContainerEnv<ContainerOperatorEnv>,
> extends Container<TEnv> {
  ...
  constructor(ctx: DurableObjectState<TEnv>, env: TEnv) {
    super(ctx, env)
    this.managedConfig = {
      ...DEFAULT_CONFIG,
    ...
    this.ctx.blockConcurrencyWhile(async () => {
      this.startRequestedAt = (await this.ctx.storage.get<number>("startRequestedAt")) ?? null
      this.containerStartedAt = (await this.ctx.storage.get<number>("containerStartedAt")) ?? null
      this.containerName = (await this.ctx.storage.get<string>("containerName")) ?? null
    })
  }
  ...
  private async captureContainerNameFromStartOptions(
    ...
    const name = startOptions?.envVars?.[this.managedConfig.containerNameEnvKey]
    if (!name) return
  ...
  private getOperator(): DurableObjectStub<ContainerOperatorInstance<TEnv>> {
    const ns = this.env.CONTAINER_OPERATOR
    const operatorId = ns.idFromName(this.managedConfig.operatorName)
    return ns.get(operatorId)
  }
  ...
  private async notifyOperator(
    ...
    if (!this.containerName) {
      console.warn("[ManagedContainer] Cannot notify operator: container name not set")
      return
    }
  ...
  async getManagedState(): Promise<ManagedContainerState> {
    ...
    const uptimeMs = this.containerStartedAt ? Date.now() - this.containerStartedAt : null
    ...
    return {
      ...
      startupDurationMs,
      startupDuration: startupDurationMs !== null ? formatDuration(startupDurationMs) : null,
      uptimeMs,
  ...
  override async onStop(stopParams: StopParams): Promise<void> {
    ...
    const uptimeMs = this.containerStartedAt ? stoppedAt - this.containerStartedAt : null

    const isGraceful = stopParams.exitCode === 0

    console.log(
      `[ManagedContainer] Container stopped at ${new Date(stoppedAt).toISOString()}` +
        (uptimeMs !== null ? ` (was alive for ${formatDuration(uptimeMs)})` : "") +
  ...
  override onError(error: unknown): void {
    ...
    if (isVersionRollout) {
      console.log("[ManagedContainer] Container stopped due to version rollout")
      this.ctx.waitUntil(this.notifyOperator("stopped", { reason: "version_rollout" }))
    } else {
  ...
  override async onActivityExpired(): Promise<void> {
    if (this.managedConfig.keepAlive) {
      ...
      console.log("[ManagedContainer] onActivityExpired intercepted — keeping container alive")
      return
    }
