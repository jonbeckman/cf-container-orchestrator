 ...
 */
export interface ContainerServiceContext<TEnv extends ManagedContainerEnv> {
 ...
 */
export interface ContainerService<TEnv extends ManagedContainerEnv> {
	/**
	...
	stop(
		name: string,
	): Effect.Effect<void, ContainerNotFoundError | ContainerStopError>;
	...
	getLiveState(
		name: string,
	): Effect.Effect<ManagedContainerState | undefined, never>;
	...
	remove(
		...
		void,
		ContainerNotFoundError | ContainerNotStoppedError | BaselineContainerError
	>;
...
export class ContainerServiceTag extends Context.Tag("ContainerService")<
	ContainerServiceTag,
	ContainerService<ManagedContainerEnv>
>() {}
 ...
 */
export const makeContainerService = <TEnv extends ManagedContainerEnv>(
	ctx: ContainerServiceContext<TEnv>,
): ContainerService<TEnv> => {
	...
	const isCrashLooping = (record: ContainerRecord): boolean => {
		...
		const oldestRelevant =
			record.stopHistory[record.stopHistory.length - crashCountToTrip]
				.timestamp;
	...
	return {
		...
		start: (
			...
			Effect.gen(function* () {
				...
				const policy =
					restartPolicy ?? existing?.restartPolicy ?? defaultRestartPolicy();
				...
				if (isHealthy && !force) {
					console.log(
						`[ContainerService] Container ${name} is already healthy`,
					);
					return;
				}
				...
				if (isHealthy && force) {
					console.log(
						`[ContainerService] Container ${name} is healthy, force starting`,
					);
				}
		...
		stop: (name: string) =>
			Effect.gen(function* () {
				...
				if (!liveState) {
					console.log(
						`[ContainerService] Container ${name} has no live state; treating as stopped`,
					);
					return;
				}
				...
				if (liveState.status === "stopping") {
					console.log(
						`[ContainerService] Container ${name} is already stopping`,
					);
					return;
				}
				...
				if (
					...
					console.log(
						`[ContainerService] Container ${name} is already stopped`,
					);
					return;
				}
		...
		remove: (name: string) =>
			Effect.gen(function* () {
				...
				if (
					...
					return yield* Effect.fail(
						ContainerNotStoppedError.of(name, liveState.status),
					);
				}
