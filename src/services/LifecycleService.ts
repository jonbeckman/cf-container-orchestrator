 ...
 */
export const makeLifecycleService = (ctx: LifecycleServiceContext): LifecycleService => {
  ...
  const recordStop = (
    ...
    const maxHistory = Math.max(0, record.restartPolicy.crash.allowedCrashes) + 1
    ...
    handleEvent: (event: LifecycleEvent) =>
      Effect.gen(function* () {
        ...
        if (event.type === "started") {
          console.log(`[LifecycleService] Container ${event.containerName} reported started`)
        ...
        if (event.type === "error") {
          console.error(`[LifecycleService] Container ${event.containerName} error: ${event.error}`)
        }
        ...
        scheduleRestart(record, `restartPolicy.when: ${record.restartPolicy.when}`)
      }),
  }
}
